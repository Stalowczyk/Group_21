...
public class Shelf {
	...
	public boolean isColumnAvailable(ArrayList<PlacedCard> chosenCards, int chosenCol) { // BISOGNA PASSARGLI ANCHE
																							// CHOSENCOLUMN
		int startPoint = 0;
		int c = 0;
		if (chosenCards != null) {
	...
	/*
	 * In questi metodi fatti molto meglio utilizzo spesso la funzione .stream
	 * quindi la spiego velocemente. Esegue di fila tutte le azioni che ho scritto,
	 * quindi per ordine: -Per ogni elemento dell'arraylist prende il CardType
	 * (.map(Card::getCardType) -Elimina risultati uguali/doppioni (.distinct)
	 * -Conta quanti CardType diversi ci sono (.count)
	 * 
	 * Quindi alla fine il .stream mi ritorna un Int del numero di CardType. Sfrutto
	 * questo dato in un valore booleano per fare 3 controlli principali: - int == 1
	 * se tutti gli elementi hanno lo stesso cardtype. - int == array.size se tutti
	 * gli elementi hanno cardtype diversi. - int >= 3/4 negli obiettivi dove ci
	 * deve essere un numero massimo di cardType.
	 * 
	...
	public boolean checkCardCount() { // Controlla tutti gli elementi della shelf
		...
		for (int i = 0; i < this.shelfLayout.length; i++) {
			for (int j = 0; j < this.shelfLayout[i].length; j++) {
				Card card = this.shelfLayout[i][j]; // Se trova una card, prende il suo cardtype e lo memorizza nella
													// hashmap
				if (card != null) {
	...
	public static boolean containsAny(List<Card> list1, List<Card> list2) { // Metodo che serve a alcuni obiettivi per
																			// controllare le carte giÃ utilizzate
		for (Card card : list1) { // Confronta 2 arraylist e ritorna true se la prima contiene almeno un elemento
									// della seconda
			if (list2.contains(card)) { // Utilizzato per vedere se una possibile forma di carte contiene delle carte
										// gia usate in altre forme (checkCube,checkCoppie)
				return true;
			}
		}
		return false;
