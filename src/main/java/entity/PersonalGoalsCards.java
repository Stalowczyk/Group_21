...
public class PersonalGoalsCards {
    ...
    /* public int score(Shelf s) {  // punteggio
        ...
        for (int i = 0; i < 6; i++) {
            ...
            PlacedCard shelfCard = s.getCard(personalCard.getCardCol(), personalCard.getCardRow());
            System.out.println("BEFORE SHELF CORDS "+ shelfCard.getCardRow()+" "+shelfCard.getCardCol());
            //0 
            if(shelfCard!=null){
    ...
    public void printOutAll(){
        for(int i = 0 ; i < currentGoalCardLayout.size();i++)
            System.out.println("PERSONALGOALCARD  LAYOUT CARD NUMBER "+currentGoalCardLayout.get(i).getCardRow()+" "+currentGoalCardLayout.get(i).getCardCol());
