...
public class PersonalGoalsCards {
	...
	public PersonalGoalsCards(int numberOfPlayers, GamePanel gp) {		//a seconda di quanti giocatori ci sono so quante goalsCards generare
		...
		for(int i = 0; i <= 6; i++) {
			for(int j = 0; j <= 5; j++) {
				if(personalGoalCardsLayout[i][j] == 1) {
					PlacedCard card = new PlacedCard(spaceIn.get(i).getCardType(), i, j, gp);	//genero una carta casuale
